...
namespace opcode {
    ...
    char _name_cmds [max_len_name_cmd][__Quantity_Types_Commands] = {
            "mov",
            "add",  "sub",
            "mul",  "div", "imul", "idiv",
            "j*",   "jmp",
...
// [IMUL]----------------------------------------------------------------------------------------------------------------
    namespace imul {
        ...
        __cmd imulr16_i8 (0x6b, 1);        // 6B /r ib	IMUL r16, r/m16, imm8	word register = r/m16 * sign-extended immediate byte
        __cmd imulr32_i8 (0x6b, 1);        // 6B /r ib	IMUL r32, r/m32, imm8	doubleword register = r/m32 * sign-extended immediate byte
        __cmd imulr64_i8 (0x6b, 1);        // 6B /r ib	IMUL r32, r/m32, imm8	doubleword register = r/m32 * sign-extended immediate byte
...
// [IDIV]----------------------------------------------------------------------------------------------------------------
    namespace idiv {
        __cmd idivrm8  (0xf638, 1);      // F6 /7	IDIV r/m8       Signed divide AX by r/m8, with result stored in AL = Quotient, AH = Remainder
        __cmd idivrm16 (0xf738, 1);      // F7 /7	IDIV r/m16      Signed divide DX:AX by r/m16, with result stored in AX = Quotient, DX = Remainder
        __cmd idivrm32 (0xf738, 1);      // F7 /7	IDIV r/m32      Signed divide EDX:EAX by r/m32, with result stored in EAX = Quotient, EDX = Remainder
        __cmd idivrm64 (0xf738, 1);      // F7 /7	IDIV r/m32      Signed divide EDX:EAX by r/m32, with result stored in EAX = Quotient, EDX = Remainder
    }
// [Jcc]----------------------------------------------------------------------------------------------------------------
    namespace jcc {
